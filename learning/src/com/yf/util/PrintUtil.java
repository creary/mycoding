...
public class PrintUtil {
	...
	public static void printHex(String tag, byte[] dataStr_cp, int start,
			int dataLen_n) {
		if (!isDebug)
			return;
		int i = 0, j = 0;
		...
		try {
			...
			for (i = 0; i < lineCnt_n; i++) {
				...
				for (j = 0; j < 16; j++) {
					if (i * 16 + j < dataLen_n) {
						if (dataStr_cp[start + i * 16 + j] < 0) {
							lineStr_ca[j * 3] = set[(byte) ((dataStr_cp[start
									+ i * 16 + j] & 0x7f + 128) >> 4)];
							lineStr_ca[j * 3 + 1] = set[(byte) ((dataStr_cp[start
									+ i * 16 + j] & 0x7f + 128) & 0xF)];
						} else {
							lineStr_ca[j * 3] = set[((byte) dataStr_cp[start
									+ i * 16 + j]) >> 4];
				...
				for (j = 0; j < 16; j++) {
					if (i * 16 + j < dataLen_n) {
						if (dataStr_cp[start + i * 16 + j] >= 0x20
								&& dataStr_cp[start + i * 16 + j] <= 0x7e
								&& dataStr_cp[start + i * 16 + j] != '%')
							lineStr_ca[50 + j] = (byte) dataStr_cp[start + i
									* 16 + j];
						else
							lineStr_ca[50 + j] = '.';
					} else
	...
	static public void printDate(String tag, long ms) {
		...
		printStr(tag, "printDate=" + String.valueOf(ms) + ";date="
